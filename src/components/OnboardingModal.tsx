...
export function OnboardingModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-[var(--bg-base)] rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-500">
        {/* Header decoration */}
          ...
          {step === 1 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="space-y-3">
                ...
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Welcome, {user?.first_name}!</h2>
                <p className="text-slate-500 dark:text-[var(--sea-ink-soft)] leading-relaxed">Let's personalize your tracking experience. We've detected you're currently at <span className="font-bold text-emerald-600">{(loadingProfile || profileLevel === 0) ? '...' : `Level ${profileLevel}`}</span>.</p>
              </div>

              <div className="space-y-6">
                <div className="space-y-4">
                  <label className="text-sm font-bold text-slate-400 dark:text-[var(--sea-ink-soft)] uppercase tracking-widest flex items-center gap-2">
                    ...
                    {[3, 4, 5, 6].map((yrs) => (
                      <button
                        ...
                        className={`py-4 rounded-2xl font-bold transition-all border-2 ${
                          ...
                          : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-emerald-200 dark:bg-[var(--sand)] dark:border-[var(--line)] dark:hover:border-emerald-500/50'
                        }`}
                      >
                        {yrs}y
                      </button>
          ...
          {step === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="space-y-3">
                ...
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Final Stretch!</h2>
                <p className="text-slate-500 dark:text-[var(--sea-ink-soft)] leading-relaxed">Since you're in your final year or have graduated, what's your current NYSC status?</p>
              </div>
                ...
                {['not started', 'serving', 'completed'].map((status) => (
                  <button
                    ...
                    className={`w-full p-5 rounded-2xl text-left font-bold transition-all border-2 flex items-center justify-between ${
                      ...
                      : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-blue-200 dark:bg-[var(--sand)] dark:border-[var(--line)] dark:hover:border-blue-500/50'
                    }`}
                  >
