...
export default function Tracker() {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-20">
      <header>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Accrual Tracker</h1>
        <p className="text-slate-500 mt-1">Visualize your debt growth and track voluntary repayments.</p>
      </header>
      ...
      <div className="bg-white border border-slate-100 rounded-[2rem] p-6 sm:p-8 shadow-sm">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Total Loan Balance</h3>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-bold text-slate-900 tracking-tighter">{formatCurrency(totalBalance - totalExtra).replace('NGN', '₦')}</span>
              {totalExtra > 0 && <span className="text-emerald-500 font-bold text-sm">(-{formatCurrency(totalExtra).replace('NGN', '₦')})</span>}
            </div>
          </div>
          <div className="hidden sm:flex items-center gap-4">
            <div className="flex items-center gap-1.5 px-3 py-1 bg-indigo-50 border border-indigo-100/50 rounded-full text-[10px] font-bold text-indigo-600 uppercase tracking-widest">
              <div className="w-2 h-2 rounded-full bg-indigo-500" />
              School Fees
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1 bg-emerald-50 border border-emerald-100/50 rounded-full text-[10px] font-bold text-emerald-600 uppercase tracking-widest">
              ...
              <div className="divide-y divide-slate-50">
                {extraPayments.map(p => (
                  <div key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                    <div>
                      <p className="text-sm font-bold text-slate-800">{formatCurrency(p.amount).replace('NGN', '₦')}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{new Date(p.date).toLocaleDateString()}</p>
                    </div>
                    <button 
          ...
          <div className="bg-blue-50 border border-blue-100 p-6 rounded-2xl space-y-4">
            ...
            <div className="space-y-3">
              <button 
                ...
                className="w-full py-3 bg-white border border-blue-200 text-blue-700 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white/50"
              >
